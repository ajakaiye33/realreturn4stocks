...
class StockCalculator:
    ...
    def time_since_purchase(self):
        ...
        if months_held < self.MIN_HOLDING_PERIOD_MONTHS:
            st.write(
                f"Note: Your holding period of {months_held} months is less than the recommended minimum of {self.MIN_HOLDING_PERIOD_MONTHS} months."
            )
    ...
    def calculate_sell_price(self, n=12):  # n=12 for monthly compounding
        ...
        self.calculate_break_even()  # Call to the new method
        max_price_per_share = (
            self.initial_price * (1 + self.target_rate / n) ** (n * years)
            + self.fees_per_share
        )
        min_price_per_share = (
            self.initial_price * (1 + real_rate / n) ** (n * years)
            + self.fees_per_share
        )
